...
namespace SlimUI.ModernMenu
{
	public class UIMenuManager : MonoBehaviour
	{
		...
		{
			AudioManager.Instance.FadeToMusic(AudioManager.Instance.gameplayMusic);
		}
		...
		IEnumerator LoadAsynchronously(string sceneName)
		{
			...
			float timer = 0f;
			float minTime = 1.5f;   // 1–2 seconds
			...
			while (!operation.isDone)
			{
				...
				// Actual load progress (0 → 0.9)
				float targetProgress = Mathf.Clamp01(operation.progress / 0.9f);

				// Smoothly animate bar (THIS FIXES THE ISSUE)
				...
				// Scene ready AND minimum time passed
